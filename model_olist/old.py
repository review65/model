...
rf_random_search = RandomizedSearchCV(
    ...
    n_iter=20, # จำนวนครั้งที่จะสุ่มลอง (ลดลงได้ถ้าช้าไป เช่น 30)
    cv=3,      # จำนวน Folds (ลดเหลือ 2 ได้ถ้าช้าไป)
    scoring='r2',
    random_state=42,
    verbose=1,
    n_jobs=-1,              
...
# --- !! สำคัญ: ใช้ X_train_scaled และ y_train_target จากขั้นตอนก่อนหน้า !! ---
rf_random_search.fit(X_train_scaled, y_train_target)
# --------------------------------------------------------------------
end_time = time.time()
print(f"RF Tuning finished in {end_time - start_time:.2f} seconds.")
print("Best RF Params:", rf_random_search.best_params_)
print("Best RF R² Score (CV):", rf_random_search.best_score_)
tuned_rf = rf_random_search.best_estimator_
tuning_results['Random Forest'] = {'model': tuned_rf, 'best_score_cv': rf_random_search.best_score_}
...
start_time = time.time()
# --- !! สำคัญ: ใช้ X_train_scaled และ y_train_target จากขั้นตอนก่อนหน้า !! ---
lgbm_random_search.fit(X_train_scaled, y_train_target)
# --------------------------------------------------------------------
end_time = time.time()
print(f"LGBM Tuning finished in {end_time - start_time:.2f} seconds.")
print("Best LGBM Params:", lgbm_random_search.best_params_)
print("Best LGBM R² Score (CV):", lgbm_random_search.best_score_)
tuned_lgbm = lgbm_random_search.best_estimator_
tuned_lgbm.set_params(
    min_gain_to_split=0.01,
    min_child_samples=50,
    subsample=0.8, subsample_freq=1,
    verbosity=-1
)
tuning_results['LightGBM'] = {'model': tuned_lgbm, 'best_score_cv': lgbm_random_search.best_score_}
...
models["Random Forest (Tuned)"] = tuned_rf

price_idx = features.index("AverageSellingPrice")
mono = [0]*len(features)
mono[price_idx] = -1

lgbm_params = tuned_lgbm.get_params()
lgbm_params.update({"monotone_constraints": mono})
models["LightGBM (Tuned)"] = LGBMRegressor(**lgbm_params)

# เตรียม target แบบ log1p สำหรับโมเดลที่เทรนบน log-space
y_train_target_log = np.log1p(y_train_target)
y_test_target_log  = np.log1p(y_test_target)

# ใช้พารามิเตอร์เดียวกับ LGBM (Tuned) + mono constraints
lgbm_log_params = dict(lgbm_params)  # copy
models["LightGBM (log1p target)"] = LGBMRegressor(**lgbm_log_params)
...
for name, model in models.items():
    ...
    if isinstance(model, LGBMRegressor):
        Xtr = pd.DataFrame(X_train_scaled, columns=features)
        Xte = pd.DataFrame(X_test_scaled, columns=features)
    else:
